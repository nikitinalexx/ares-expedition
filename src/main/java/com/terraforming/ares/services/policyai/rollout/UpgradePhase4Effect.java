...
public class UpgradePhase4Effect extends AbstractTagEffect {

    private final List<Integer> phaseInput;
    private final Card playedCard;

    public UpgradePhase4Effect(Card playedCard, Map<Integer, List<Integer>> params) {
    ...
    public boolean isPresent() {
        return playedCard.getCardMetadata().getCardAction() == CardAction.UPDATE_PHASE_4_CARD;
    }

    @Override
    public void prepareContext(PolicyRecord ctx) {
        ctx.choosingPhase4Upgrade = true;
    }

    @Override
    public void clearContext(PolicyRecord ctx) {
        ctx.choosingPhase4Upgrade = false;
    }
