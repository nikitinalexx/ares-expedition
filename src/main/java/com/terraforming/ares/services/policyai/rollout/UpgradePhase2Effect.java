...
public class UpgradePhase2Effect extends AbstractTagEffect {

    private final List<Integer> phaseInput;
    private final Card playedCard;

    public UpgradePhase2Effect(Card playedCard, Map<Integer, List<Integer>> params) {
    ...
    public boolean isPresent() {
        return playedCard.getCardMetadata().getCardAction() == CardAction.UPDATE_PHASE_2_CARD;
    }

    @Override
    public void prepareContext(PolicyRecord ctx) {
        ctx.choosingPhase2Upgrade = true;
    }

    @Override
    public void clearContext(PolicyRecord ctx) {
        ctx.choosingPhase2Upgrade = false;
    }
