...
public class Network2ProjectBuildService {
    ...
    public List<CardWithChanceModifier> getBestCardProjectionsIgnoreRequirements(MarsGame marsGame, Player player, List<Card> cards) {
        ...
        List<CardWithInputParams> optimalInputsForCards = aiOptimalBuildService.getOptimalInputsForCards(marsGame, player, cards, null);
        ...
        for (CardWithInputParams cardWithInputParams : optimalInputsForCards) {
            ...
            if (card.getClass() == SyntheticCatastrophe.class) {
                for (Map<Integer, List<Integer>> redCardInputParams : inputParamsVariations) {
                    int redCardInput = redCardInputParams.get(InputFlag.SYNTHETIC_CATASTROPHE_CARD.getId()).getFirst();
                    Card redCard = cardService.getCard(redCardInput);

                    MegacreditsPayment redCardMcPayment = network2PaymentService.getMcPaymentOnly(marsGame, player, redCard, redCardInputParams);

                    MarsGame gameCopy = projectBuildRedCardFromSyntheticCatastrophe(marsGame, player, cardService.getCard(redCardInput), redCardInputParams, redCardMcPayment);
                    buildProjections.add(getGameParams(gameCopy, player.getUuid()));
                }
            } else {
    ...
    private MarsGame projectBuildDummyCard(MarsGame game, Player player, Integer originalCardId, Integer dummyCardId) {
        ...
        int originalPlayerHandSize = player.getHand().size() - 1;
    ...
    public MarsGame projectBuildRedCardFromSyntheticCatastrophe(MarsGame game, Player player, Card redCard, Map<Integer, List<Integer>> inputParameters, MegacreditsPayment payment) {
        game = new MarsGame(game);
        player = game.getPlayerByUuid(player.getUuid());

        player.getPlayed().removeCard(redCard.getId());

        int originalPlayerHandSize = player.getHand().size();

        aiTurnService.buildBlueRedProjectSyncNoRequirements(game, player, redCard.getId(), List.of(payment), inputParameters);
        aiUtility.simulateDummyHandInsteadOfNewCards(player, originalPlayerHandSize);
    ...
    public MarsGame projectBuildCardNoRequirements(MarsGame game, Player player, Card card, Map<Integer, List<Integer>> inputParameters, MegacreditsPayment payment) {
        ...
        int originalPlayerHandSize = player.getHand().size() - (player.getHand().containsCard(card.getId()) ? 1 : 0);
