...
public class AiOptimalBuildService {
    ...
    public List<CardWithInputParams> getOptimalInputsForCards(
            MarsGame game, Player player, List<Card> cards) {
        ...
        OptimizedInputDecisions optimizedDecisions = optimizer.optimizeInputDecisions(game, player, analysis);
    ...
    public CardWithInputParams generateInputBasedOnOptimizedDecisions(
        ...
        if (cardAction == CardAction.SYNTHETIC_CATASTROPHE) {
            List<Card> playedRedCards = player.getPlayed().getCards().stream()
                    .map(cardService::getCard)
                    .filter(c -> c.getColor() == CardColor.RED)
                    .toList();
            if (playedRedCards.isEmpty()) {
                throw new IllegalStateException("Trying to use synthetic catastrophe without any red cards");
            }


            return new CardWithInputParams(card, Map.of(InputFlag.SYNTHETIC_CATASTROPHE_CARD.getId(),
                    List.of(playedRedCards.getFirst().getId())));
