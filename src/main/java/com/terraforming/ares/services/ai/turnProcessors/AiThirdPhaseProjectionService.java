...
//        }
        ...
        if (!CollectionUtils.isEmpty(activeCards)) {
            ...
            for (Card activeCard : activeCards) {
                ...
                float futureState = deepNetwork.testState(datasetCollectionService.collectGameAndPlayers(gameCopy, playerCopy, anotherPlayer).applyDifference(difference).applyOpponentDifference(opponentDifference), player.isFirstBot() ? 1 : 2);
        ...
        float futureState = deepNetwork.testState(datasetCollectionService.collectGameAndPlayers(gameCopy, playerCopy, anotherPlayer).applyDifference(initialDifference).applyOpponentDifference(opponentDifference), player.isFirstBot() ? 1 : 2);
