...
public class ModproCorporationActionProcessor implements BlueActionCardProcessor<ModproCorporation> {
    ...
    public TurnResponse process(MarsGame game, Player player, Card actionCard) {
        ...
        for (Integer card : cardService.dealCards(game, 4)) {
            Card projectCard = cardService.getCard(card);

            if (projectCard.getTags().contains(player.getCardToTag().get(ModproCorporation.class).get(0))
                    || projectCard.getTags().contains(Tag.DYNAMIC)) {
