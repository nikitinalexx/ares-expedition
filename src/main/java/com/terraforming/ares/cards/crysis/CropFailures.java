 ...
 */
public class CropFailures implements CrysisCard {

    @Getter
    private final int id;

    public CropFailures(int id) {
        this.id = id;
    }
    ...
    public CrysisCardAction cardAction() {
        return CrysisCardAction.CROP_FAILURES;
    }
    ...
    public String immediateEffect() {
        return "Draw a card. Decrease the oxygen 2 steps.";
    }
    ...
    public void postProjectBuiltEffect(MarsContext marsContext, Card card, Map<Integer, List<Integer>> inputParams) {
        int tagsCount = marsContext.getCardService().countCardTags(card, Set.of(Tag.PLANT), inputParams);
